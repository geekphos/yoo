...
var (
	...
	// ErrSignToken 表示签发 JWT AccessToken 时出错.
	ErrSignToken = &Errno{HTTP: 401, Code: "AuthFailure.SignTokenError", Message: "Error occurred while signing the JSON web token."}

	// ErrTokenInvalid 表示 JWT AccessToken 格式错误.
	ErrTokenInvalid = &Errno{HTTP: 401, Code: "AuthFailure.TokenInvalid", Message: "AccessToken was invalid."}

	// ErrBadRequest 表示所有的请求参数错误.
	ErrBadRequest = &Errno{HTTP: 400, Code: "InvalidParameter.BadRequest", Message: "Bad request."}
)
